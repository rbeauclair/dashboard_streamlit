...
# mantém em cache para evitar múltiplas conversões
@st.cache_data
def converte_csv(df):
...
with st.sidebar.expander('Nome do produto'):
    produtos = st.multiselect('Selectione os produtos', dados['Produto'].unique(), dados['Produto'].unique())
...
with st.sidebar.expander('Preço do produto'):
    preco = st.slider('Selectione os preço', 0, 5000, (0,5000))
...
with coluna1:
    nome_arquivo = st.text_input('', label_visibility = 'collapsed', value = 'resultados')
    nome_arquivo += '.csv'
with coluna2:
    st.download_button('Fazer o download dos resultados em csv', 
